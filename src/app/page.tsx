...
<div className="flex min-h-screen items-center justify-center bg-zinc-50 font-sans dark:bg-black text-black dark:text-white">
  ...
  <title>{ "{{ $json.seo_title || 'POTransfora | Digital Sovereignty' }}" }</title>
  <meta name="description" content={ "{{ $json.seo_description || 'Leading the future with the Organizational DNA Protocol' }}" } />
  ...
  <main className="flex min-h-screen w-full max-w-3xl flex-col items-center justify-between py-32 px-16 bg-white dark:bg-black sm:items-start relative overflow-hidden">
    ...
    {/* Sovereign Branding Header (The Minimalism Path) */}
    ...
    {/* Action Zone (The Sovereign Interaction) */}
    <div className="flex flex-col gap-5 sm:flex-row mt-12 w-full sm:w-auto">
      <a
        className="flex h-14 items-center justify-center rounded-full bg-[#00A3FF] px-10 text-sm font-bold text-white transition-all hover:bg-[#0082CC] hover:shadow-[0_0_20px_rgba(0,163,255,0.3)]"
        href="#"
      >
        استكشف السيادة
      </a>
      <div className="flex items-center justify-center text-zinc-400 text-[10px] tracking-[0.3em] uppercase italic">
         STATUS: { "{{ $json.seo_status || 'Active' }}" }
      </div>
    </div>

    {/* Visual Identity - The Blue Cape Corner */}
    <div className="absolute top-0 right-0 w-40 h-40 bg-[#00A3FF] opacity-[0.03] rounded-bl-[100px]"></div>
  </main>
</div>
);
}
